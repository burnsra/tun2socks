...
public final class TSIPStack {
    ...
    let processQueue = DispatchQueue(label: "tun2socks.IPStackQueue", attributes: [])
    ...
    public func resumeTimer() {
        timer = DispatchSource.makeTimerSource(flags: DispatchSource.TimerFlags(rawValue: UInt(0)), queue: processQueue) /*Migrator FIXME: Use DispatchSourceTimer to avoid the cast*/ as! DispatchSource
        // note the default tcp_tmr interval is 250 ms.
        // I don't know the best way to set leeway.
        timer!.setTimer(start: DispatchTime.now(), interval: NSEC_PER_SEC / 4, leeway: NSEC_PER_SEC / 4)
     ...
     */
    public func receivedPacket(_ data: Data) {
        dispatch_call {
            ...
            let buf = pbuf_alloc(PBUF_RAW, UInt16(data.count), PBUF_RAM)
            (data as NSData).getBytes((buf?.pointee.payload)!, length: data.count)
            ...
            netif_list.pointee.input(buf, self.interface)
        }
    }

    func writePBuf(_ buf: UnsafeMutablePointer<pbuf>) {
